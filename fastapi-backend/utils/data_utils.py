...
def deprocess(x):
    ...
    x = x.astype(np.uint8)
    if x.shape[-1] != 3:
        print("WARNING: Unexpected channel count after deprocess:", x.shape)
    return x

def preprocess(x):
    # If x is a NumPy array (i.e., already an image), skip reading from disk
...
def read_and_resize_from_array(img_array, img_res):
    """Resize image from numpy array input (for video frames)"""
    try:
        ...
        print(f"Error in read_and_resize_from_array: {str(e)}")
        return None
